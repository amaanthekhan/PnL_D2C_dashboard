...
with hc2:
    ...
    if use_custom_dates:
        ...
        timeframe_label = f"{start_date.strftime('%d %b %Y')} → {end_date.strftime('%d %b %Y')} ({num_days} days)"
        ...
        with mc2:
            sel_year  = st.number_input("Year", min_value=2000, max_value=2100, value=date.today().year, step=1)
        month_idx     = month_names.index(sel_month) + 1
        if month_idx == 12:
            last_day = (date(sel_year + 1, 1, 1) - timedelta(days=1)).day
        else:
            last_day = (date(sel_year, month_idx + 1, 1) - timedelta(days=1)).day
        num_days        = last_day
        days_factor     = 1.0
        timeframe_label = f"{sel_month} {sel_year}"
...
with col3:
    ...
    # ── Marketing input mode toggle ───────────────────────────────
    mkt_mode = st.toggle("Enter marketing as cost per placed order", value=False,
                         help="OFF = enter total marketing spend | ON = enter cost per placed order")
    if mkt_mode:
        mkt_per_placed = st.number_input("Marketing Cost per Placed Order (₹)", min_value=0.0,
                                          value=40.0, step=1.0)
...
st.caption("RTO (Return to Origin) — the silent killer of D2C margins. Track it carefully.")
...
delivered = int(shipped) - int(rto_orders)
...
m2.metric(
    f"Total Profit — {timeframe_label}",
    f"₹{total_profit:,.0f}",
...
# ── Full-width: RTO Sensitivity ───────────────────────────────────
st.markdown("**What happens to profit if RTO rate changes?**")
st.caption("This one chart can save a brand. Slide your eyes to the current RTO marker and see how close you are to the red zone.")
...
fig_sens.add_trace(go.Scatter(
    ...
    marker=dict(size=8, color=[DKGREEN if (p is not None and p >= 0) else RED for p in profit_rto]),
...
fig_sens.add_vline(
    x=rto_rate_pct, line_dash="dot", line_color=AMBER,
...
def generate_pdf():
    ...
    title_text = company_name.strip() if company_name.strip() else "D2C P&L Report"
    ...
    pl_rows = [
        ("Selling Price",                          sp),
        ("Marketing Cost (adjusted)",              mkt_per_del),
        ("Forward + Reverse Shipping (adjusted)",  ship_per_del),
        ("Cost Price",                             cp),
        ("Total Cost per Delivered Order",         total_cost),
        ("Profit per Delivered Order",             profit),
    ]
    for label, val in pl_rows:
        is_profit_row = "Profit" in label
        is_total_row  = "Total Cost" in label
        if is_profit_row or is_total_row:
            pdf.set_font("Helvetica", "B", 11)
        else:
            pdf.set_font("Helvetica", "", 11)
        pdf.set_text_color(80, 80, 80)
        ...
        if is_profit_row:
            pdf.set_text_color(15, 110, 86) if val >= 0 else pdf.set_text_color(153, 60, 29)
        else:
            pdf.set_text_color(30, 30, 30)
    ...
    pdf.set_font("Helvetica", "", 11)
    summary_rows = [
        (f"Profit per Order",                    f"Rs {profit:,.2f}  ({profit_pct:.1f}% of SP)"),
    ...
    pdf.cell(0, 6, "Generated by D2C P&L Dash  ·  built by Amaan Khan  ·  amaankhan2203@gmail.com", ln=True, align="C")
...
if st.button("Download Report as PDF"):
    if not company_name.strip():
        ...
        file_name = f"PnL_{company_name.strip().replace(' ','_')}_{timeframe_label.replace(' ','_').replace('→','to')}.pdf"
        st.download_button(
